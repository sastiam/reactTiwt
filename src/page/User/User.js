...
function User(props) {
    ...
    useEffect(() => {
        getUserApi(params.id).then(response => {
            ...
            setUser(response);
            if(!response) toast.error("El usuario visitado no existe.")
        }).catch(() => {
    ...
    return (
        <BasicLayout className="user" setRefreshCheckLogin={setRefreshCheckLogin}>
            <div className="user__title">
                <h2>{user ? `${user.nombre} ${user.apellidos}` : "Usuario no existe"}</h2>
            </div>
            <BannerAvatar user={user} loggedUser={loggedUser}></BannerAvatar>
            <InfoUser user={user}></InfoUser>
            <div className="user__tweets">
                <h3>Tweets</h3>
                {tweets && <ListTweets tweets={tweets} user={user} loggedUser={loggedUser}></ListTweets>}
